...
def _print_consumer(arg):
    i, n_iter = arg
    print(f"Iteration-residual {i}/{n_iter}")
...
def progress_bar(*arg):
    ...
    i, n_iter = arg
    result = host_callback.id_tap(_print_consumer, (i, n_iter))
    return result
...
def _cg_solve(A, b, x0=None, *, maxiter, tol=1e-5, atol=0.0, M=_identity):
    ...
    converged = rs <= atol2
    # all_param: the second is residual, the last one is iterations
    # test=progress_bar(all_param[1],all_param[-1])
    info = {"error": rs, "niter": k}  #
    return x_final, info
...
def _bicgstab_solve(A, b, x0=None, *, maxiter, tol=1e-5, atol=0.0, M=_identity):
    ...
    x_final, r, _, gamma, _, _, _, _, k = lax.while_loop(
...
def _gmres_batched(A, b, x0, unit_residual, residual_norm, ptol, restart, M):
    ...
    beta_vec = (
        jnp.zeros_like(H, shape=(restart + 1,)).at[0].set(residual_norm.astype(dtype))
    )
...
#     return jnp.logical_and(k < maxiter, residual_norm > )
